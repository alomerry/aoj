...
CREATE TABLE IF NOT EXISTS `contest` (
  ...
  `start_time` datetime NOT NULL COMMENT '开始时间(年月日时分)',
  `end_time` datetime NOT NULL COMMENT '结束时间(年月日时分)',
...
INSERT INTO `contest` (`contest_id`, `title`, `access`, `user_id`, `start_time`, `end_time`, `describes`, `privates`, `organizer`, `max`, `now`) VALUES
...
) ENGINE=InnoDB AUTO_INCREMENT=9 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='比赛申请表';
...
) ENGINE=InnoDB AUTO_INCREMENT=20956 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='竞赛题目';

-- 正在导出表  online_judge.contest_problem 的数据：~3 rows (大约)
...
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='用户分组';
...
CREATE TABLE IF NOT EXISTS `problems` (
  ...
  `hint` char(1) COLLATE utf8_bin NOT NULL DEFAULT '0' COMMENT '暗示',
  ...
  KEY `FK_problems_users` (`user_id`),
  CONSTRAINT `FK_problems_users` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`)
) ENGINE=InnoDB AUTO_INCREMENT=20955 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='题目表';

-- 正在导出表  online_judge.problems 的数据：~4 rows (大约)
/*!40000 ALTER TABLE `problems` DISABLE KEYS */;
INSERT INTO `problems` (`problem_id`, `title`, `defunct`, `user_id`, `hint`, `create_time`, `source`, `description`, `click`, `accepted`, `submit`, `sample_input`, `sample_output`, `memory_limit`, `time_limit`, `output`, `input`) VALUES
	(1, 'A+B 输入输出练习I', '3', 12, '0', '2017-01-28 09:05:58', 'Codeup', '你的任务是计算a+b。这是为了acm初学者专门设计的题目。你肯定发现还有其他题目跟这道题的标题类似，这些问题也都是专门为初学者提供的。', 0, 0, 0, '<pre>1 5\r\n10 20</pre>', '<pre><span>6\r\n30</span></pre>', 16, 500, '对于输入的每对a和b，你需要依次输出a、b的和。<br>\r\n如对于输入中的第二对a和b，在输出中它们的和应该也在第二行。', '输入包含一系列的a和b对，通过空格隔开。一对a和b占一行。'),
	(2, 'A+B', '1', 12, '0', '2018-01-28 09:06:01', NULL, '666', 0, 0, 0, '666', '666', 16, 500, '666', '666'),
	(6, '陶陶摘苹果', '1', 12, '0', '2019-01-28 09:06:04', NULL, '陶陶家的院子里有一棵苹果树，每到秋天树上就会结出10个苹果。苹果成熟的时候，陶陶就会跑去摘苹果。陶陶有个30厘米高的板凳，当她不能直接用手摘到苹果的时候，就会踩到板凳上再试试。', 0, 0, 0, '<pre>100 200 150 140 129 134 167 198\r\n200 111\r\n110\r\n</pre>', '<pre>5\r\n</pre>', 16, 500, '每组输出包括一行，这一行只包含一个整数，表示陶陶能够摘到的苹果的数目。', '每组输入数据包括两行。第一行包含10个100到200之间（包括100和200）的整数（以厘米为单位）分别表示10个苹果到地面的高度，两个相邻的整数之间用一个空格隔开。第二行只包括一个100到120之间（包含100和120）的整数（以厘米为单位），表示陶陶把手伸直的时候能够达到的最大高度。');
...
-- 正在导出表  online_judge.users 的数据：~5 rows (大约)
...
INSERT INTO `users` (`user_id`, `username`, `nickname`, `passwd`, `school`, `email`, `remark`, `submit`, `solved`, `access_time`, `session_id`) VALUES
	(9, 'cloud', '吱吱云', '5cba13819e624f8dc0a991a7691f3f82', 'hyit', '3122172709@qq.com', NULL, 0, 0, '2018-01-16 19:18:22', '77868C0C384CF65522ABCE4E8DDFDC19'),
	(10, 'morizunzhu', '就当一次路过丶', '5cba13819e624f8dc0a991a7691f3f82', 'hyit', 'morizunzhu@163.com', NULL, 0, 0, '2019-01-16 19:18:27', 'C6E30682C258208896F71D66A5C837AB'),
	(11, 'wu1jin2cheng3', '末日尊主恋火', '5cba13819e624f8dc0a991a7691f3f82', 'hyit', 'wu1jin2cheng3@live.cn', NULL, 0, 0, '2017-01-16 19:18:28', NULL),
	(12, 'admin', 'admin', '5cba13819e624f8dc0a991a7691f3f82', 'hyit', '940406032@qq.com', NULL, 0, 0, '2015-01-16 19:18:29', 'E3382BCF1C37C8F86508CDF92414A71D'),
